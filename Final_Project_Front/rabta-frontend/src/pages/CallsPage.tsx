...
export const CallsPage: React.FC = () => {
  ...
  // 💡 ملاحظة: استبدل هذا بالـ ID الحقيقي من الـ AuthContext لاحقاً
  const myUserId = "65a1234567890abcdef12345"; 

  // --- Functions ---
  const getOtherUser = useCallback((call: CallRecord): User => {
    return call.caller._id === myUserId ? call.receiver : call.caller;
  }, [myUserId]);
    ...
    const fetchHistory = async (): Promise<void> => {
      try {
        ...
        const response: AxiosResponse<ApiResponse> = await axios.get(
          'http://localhost:5000/api/v1/calls/history',
        ...
        if (fetchedData.length > 0) {
          setSelectedCallId(fetchedData[0]._id);
        }
      } catch (err: unknown) {
